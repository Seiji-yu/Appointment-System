...
var H = k;
var G = { isSpaceSeparator(D) {
  return typeof D == "string" && H.Space_Separator.test(D);
}, isIdStartChar(D) {
  return typeof D == "string" && (D >= "a" && D <= "z" || D >= "A" && D <= "Z" || D === "$" || D === "_" || H.ID_Start.test(D));
}, isIdContinueChar(D) {
  return typeof D == "string" && (D >= "a" && D <= "z" || D >= "A" && D <= "Z" || D >= "0" && D <= "9" || D === "$" || D === "_" || D === "\u200C" || D === "\u200D" || H.ID_Continue.test(D));
}, isDigit(D) {
...
var M;
var O;
var Cu = function(C2, E) {
  q = String(C2), h = "start", y = [], V = 0, S = 1, g = 0, l = void 0, M = void 0, O = void 0;
  do
    l = ru(), nu[h]();
  while (l.type !== "eof");
  return typeof E == "function" ? T({ "": O }, "", E) : O;
};
function T(D, C2, E) {
  const n = D[C2];
  if (n != null && typeof n == "object") if (Array.isArray(n)) for (let m = 0; m < n.length; m++) {
    const o = String(m), x = T(n, o, E);
    x === void 0 ? delete n[o] : Object.defineProperty(n, o, { value: x, writable: true, enumerable: true, configurable: true });
  }
  else for (const m in n) {
    const o = T(n, m, E);
    o === void 0 ? delete n[m] : Object.defineProperty(n, m, { value: o, writable: true, enumerable: true, configurable: true });
  }
  return E.call(D, C2, n);
}
var t;
var e;
var $;
...
function u() {
  const D = b();
  return D === `
` ? (S++, g = 0) : D ? g += D.length : g++, D && (V += D.length), D;
}
var U = { default() {
  switch (r) {
    case "	":
...
`:
  ...
  switch (D) {
    ...
    case "\u200D":
      break;
    default:
      if (!f.isIdContinueChar(D)) throw W();
      break;
  }
  e += D, t = "identifierName";
}, sign() {
...
function i(D, C2) {
  return { type: D, value: C2, line: S, column: g };
}
function P(D) {
  for (const C2 of D) {
    if (b() !== C2) throw a(u());
...
function Eu() {
  let D = "", C2 = b();
  if (!f.isHexDigit(C2) || (D += u(), C2 = b(), !f.isHexDigit(C2))) throw a(u());
...
function K() {
  ...
  if (O === void 0) O = D;
    ...
    const C2 = y[y.length - 1];
    Array.isArray(C2) ? C2.push(D) : Object.defineProperty(C2, M, { value: D, writable: true, enumerable: true, configurable: true });
  }
  if (D !== null && typeof D == "object") y.push(D), Array.isArray(D) ? h = "beforeArrayValue" : h = "beforePropertyName";
  else {
    const C2 = y[y.length - 1];
    C2 == null ? h = "end" : Array.isArray(C2) ? h = "afterArrayValue" : h = "afterPropertyValue";
  }
}
function _() {
  y.pop();
...
function a(D) {
  return J(D === void 0 ? `JSON5: invalid end of input at ${S}:${g}` : `JSON5: invalid character '${Y(D)}' at ${S}:${g}`);
}
function I() {
...
function iu(D) {
  console.warn(`JSON5: '${Y(D)}' in strings is not valid ECMAScript; consider escaping`);
}
function Y(D) {
  const C2 = { "'": "\\'", '"': '\\"', "\\": "\\\\", "\b": "\\b", "\f": "\\f", "\n": "\\n", "\r": "\\r", "	": "\\t", "\v": "\\v", "\0": "\\0", "\u2028": "\\u2028", "\u2029": "\\u2029" };
  if (C2[D]) return C2[D];
...
function J(D) {
  const C2 = new SyntaxError(D);
  return C2.lineNumber = S, C2.columnNumber = g, C2;
}
var au = X(Cu);
var Q = G;
var Bu = function(C2, E, n) {
  const m = [];
  let o = "", x, L, N2 = "", R;
  ...
  return n instanceof Number ? n = Number(n) : n instanceof String && (n = String(n)), typeof n == "number" ? n > 0 && (n = Math.min(10, Math.floor(n)), N2 = "          ".substr(0, n)) : typeof n == "string" && (N2 = n.substr(0, 10)), z("", { "": C2 });
  function z(A, B) {
    let F = B[A];
...
` + B + "}";
  ...
  function Du(A) {
    ...
    if (!Q.isIdStartChar(B)) return j(A);
    for (let F = B.length; F < A.length; F++) if (!Q.isIdContinueChar(String.fromCodePoint(A.codePointAt(F)))) return j(A);
    return A;
  }
  function eu(A) {
    if (m.indexOf(A) >= 0) throw TypeError("Converting circular structure to JSON5");
...
var su = X(Bu);
function ou(D, C2) {
  const E = au(D, C2?.reviver);
  return N(D, E, C2), E;
}
function cu(D, C2) {
  const E = C(D, C2), n = su(D, C2?.replacer, E.indent);
...
//# sourceMappingURL=chunk-2RTYPKD3.js.map